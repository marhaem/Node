...
internals.fillMissedErrors = function(errors, requireds, state, options) {
  ...
  if (knownMisses.length) {
    if (unknownMisses) {
      ...
      errors.push(this.createError('array.includesRequiredKnowns', {knownMisses: knownMisses}, {
        key: state.key,
        path: state.path
      }, options));
    }
  } else {
    errors.push(this.createError('array.includesRequiredUnknowns', {unknownMisses: unknownMisses}, {
      key: state.key,
      path: state.path
    }, options));
  }
