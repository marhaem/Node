...
internals.Object.prototype._base = function(value, state, options) {
  ...
  const childOptions = options.language && options.language.label ? Hoek.applyToDefaults(options, {language: {label: null}}, true) : options;
  ...
  if ((this._inner.children || this._inner.patterns.length) && unprocessedKeys.length) {
    ...
    if (unprocessedKeys.length && (this._flags.allowUnknown !== undefined ? !this._flags.allowUnknown : !options.allowUnknown)) {
      for (let i = 0; i < unprocessedKeys.length; ++i) {
        errors.push(this.createError('object.allowUnknown', null, {
          key: unprocessedKeys[i],
          path: state.path + (state.path ? '.' : '') + unprocessedKeys[i]
        }, childOptions));
      }
    }
  }
  for (let i = 0; i < this._inner.dependencies.length; ++i) {
    ...
    const err = internals[dep.type].call(this, dep.key !== null && value[dep.key], dep.peers, target, {
      key: dep.key,
