...
const Isemail = require('../lib/index');
const Tests = require('./tests.json!systemjs-json');
const internals = {defaultThreshold: 16};
...
const tldExpectations = [['shouldbe@invalid', diag.errUnknownTLD], ['shouldbe@example.com', diag.valid]];
describe('validate()', () => {
  it('should check options.tldWhitelist', (done) => {
    ...
    expect(() => {
      Isemail.validate('', {tldWhitelist: 77});
  ...
  it('should check options.tldBlacklist', (done) => {
    ...
    expect(() => {
      Isemail.validate('', {tldBlacklist: 77});
    }).to.throw(/tldBlacklist/);
    done();
  });
  it('should check options.minDomainAtoms', (done) => {
    expect(() => {
      Isemail.validate('person@top', {minDomainAtoms: -1});
    }).to.throw(/minDomainAtoms/);
    expect(() => {
      Isemail.validate('person@top', {minDomainAtoms: 1.5});
    }).to.throw(/minDomainAtoms/);
    done();
  });
  it('should use options.errorLevel', (done) => {
    expect(Isemail.validate('person@123', {errorLevel: diag.rfc5321TLDNumeric + 1})).to.equal(0);
    expect(Isemail.validate('person@123', {errorLevel: diag.rfc5321TLDNumeric})).to.equal(diag.rfc5321TLDNumeric);
    done();
  });
  it('should ensure callback provided with checkDNS', (done) => {
    expect(() => {
      Isemail.validate('person@top', {checkDNS: true});
