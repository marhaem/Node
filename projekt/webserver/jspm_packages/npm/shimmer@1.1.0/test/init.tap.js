...
test('shimmer initialization', function(t) {
  ...
  var mock = sinon.expectation.create('logger').withArgs('no original function undefined to wrap').once();
  t.doesNotThrow(function() {
    shimmer({logger: mock});
...
test('shimmer initialized with non-function logger', function(t) {
  t.plan(2);
  var mock = sinon.expectation.create('logger').withArgs("new logger isn't a function, not replacing").once();
  shimmer({logger: mock});
  t.doesNotThrow(function() {
