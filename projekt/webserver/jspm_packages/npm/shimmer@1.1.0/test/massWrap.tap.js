...
test('wrap called with module but nothing else', function(t) {
  t.plan(2);
  var mock = sinon.expectation.create('logger').withExactArgs('must provide one or more functions to wrap on modules').once();
  shimmer({logger: mock});
  t.doesNotThrow(function() {
...
test('wrap called with non-function wrapper', function(t) {
  t.plan(2);
  var mock = sinon.expectation.create('logger').withArgs('must provide one or more functions to wrap on modules').once();
  shimmer({logger: mock});
  t.doesNotThrow(function() {
