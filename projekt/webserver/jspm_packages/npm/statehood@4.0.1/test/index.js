...
(function(Buffer) {
  ...
  const Statehood = require('../lib/index');
  const internals = {};
  ...
  describe('Definitions', () => {
    ...
    describe('format()', () => {
      ...
      it('formats a header with server definition (iron + options, buffer password)', (done) => {
        ...
        definitions.add('sid', {
          encoding: 'iron',
          password: Cryptiles.randomBits(256),
          iron: Iron.defaults
        });
        definitions.format({
          name: 'sid',
      ...
      it('fails on too long cookie domain', (done) => {
        ...
        definitions.format({
          ...
          options: {
            ...
            isHttpOnly: false,
            path: '/',
            domain: '1234567890123456789012345678901234567890123456789012345678901234567890.example.com'
          }
        }, (err, header) => {
          expect(err).to.exist();
      ...
      it('formats a header with cookie domain with . prefix', (done) => {
        ...
        definitions.format({
          ...
          options: {
            ...
            isHttpOnly: false,
            path: '/',
            domain: '.12345678901234567890.example.com'
          }
        }, (err, header) => {
          expect(err).to.not.exist();
          done();
