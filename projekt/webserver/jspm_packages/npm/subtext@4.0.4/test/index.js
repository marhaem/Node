...
(function(Buffer, process) {
  ...
  const Subtext = require('../lib/index');
  ...
  describe('parse()', () => {
    ...
    it('errors on invalid content type header', (done) => {
      const payload = '{"x":"1","y":"2","z":"3"}';
      const request = Wreck.toReadableStream(payload);
      request.headers = {'content-type': 'steve'};
      Subtext.parse(request, null, {
        parse: true,
        output: 'data'
      }, (err, parsed) => {
    ...
    it('errors on unsupported content type', (done) => {
      const payload = '{"x":"1","y":"2","z":"3"}';
      const request = Wreck.toReadableStream(payload);
      request.headers = {'content-type': 'james/bond'};
      Subtext.parse(request, null, {
        parse: true,
        output: 'data'
      }, (err, parsed) => {
    ...
    it('parses a multipart payload', (done) => {
      ...
      const request = Wreck.toReadableStream(payload);
      request.headers = {'content-type': 'multipart/form-data; boundary=AaB03x'};
      Subtext.parse(request, null, {
        parse: true,
        output: 'data'
      }, (err, parsed) => {
    ...
    it('parses multiple files as streams', (done) => {
      ...
      Subtext.parse(request, null, {
        ...
        expect(err).to.not.exist();
        expect(parsed.payload.files[0].hapi).to.equal({
          filename: 'file1.txt',
          headers: {
            'content-disposition': 'form-data; name="files"; filename="file1.txt"',
            'content-type': 'text/plain'
        ...
        expect(parsed.payload.files[1].hapi).to.equal({
          filename: 'file2.txt',
          headers: {
            'content-disposition': 'form-data; name="files"; filename="file2.txt"',
            'content-type': 'text/plain'
        ...
        expect(parsed.payload.files[2].hapi).to.equal({
          filename: 'file3.txt',
          headers: {
            'content-disposition': 'form-data; name="files"; filename="file3.txt"',
            'content-type': 'text/plain'
