...
(function(Buffer) {
  ...
  describe('Plugin', () => {
    describe('select()', () => {
      it('creates a subset of connections for manipulation', (done) => {
        ...
        register.attributes = {name: 'plugin'};
    ...
    describe('register()', () => {
      ...
      it('registers a required plugin', (done) => {
        const server = new Hapi.Server();
        server.connection({labels: ['a', 'b']});
        const test = {register: function(srv, options, next) {
            expect(options.something).to.be.true();
            return next();
          }};
        test.register.attributes = {name: 'test'};
        server.register({
          register: test,
          options: {something: true}
        }, (err) => {
      ...
      it('throws on bad plugin (missing attributes)', (done) => {
        const server = new Hapi.Server();
        expect(() => {
          server.register({register: function(srv, options, next) {
              return next();
            }}, Hoek.ignore);
        }).to.throw();
        done();
      ...
      it('sets version to 0.0.0 if missing', (done) => {
        ...
        test.attributes = {pkg: {name: 'steve'}};
        const server = new Hapi.Server();
        server.connection();
        server.register(test, (err) => {
          expect(err).to.not.exist();
          ...
          server.inject('/', (res) => {
            expect(res.result).to.equal(require('../package.json!systemjs-json').version);
            done();
          });
        });
      });
      ...
      it('registers a plugin with routes path prefix (plugin options)', (done) => {
        ...
        server.connection({labels: 'test'});
        server.register({
          register: internals.plugins.test1,
          routes: {prefix: '/abc'}
        }, {routes: {prefix: '/xyz'}}, (err) => {
      ...
      it('registers a child plugin with parent routes path prefix and inner register prefix', (done) => {
        ...
        server.connection({labels: 'test'});
        server.register({
          register: internals.plugins.child,
          options: {routes: {prefix: '/inner'}}
        }, {routes: {prefix: '/xyz'}}, (err) => {
          expect(err).to.not.exist();
      ...
      it('registers a child plugin with parent routes vhost prefix and inner register vhost', (done) => {
        ...
        server.register({
          register: internals.plugins.child,
          options: {routes: {vhost: 'example.net'}}
        }, {routes: {vhost: 'example.com'}}, (err) => {
          expect(err).to.not.exist();
          server.inject({
            url: '/test1',
            headers: {host: 'example.com'}
      ...
      it('adds auth strategy via plugin', (done) => {
        ...
        server.register(internals.plugins.auth, (err) => {
          ...
          server.select('a').inject('/', (res1) => {
            ...
            server.select('a').inject({
              method: 'GET',
              url: '/',
              headers: {authorization: 'Basic ' + (new Buffer('john:12345', 'utf8')).toString('base64')}
            }, (res2) => {
    ...
    describe('ext()', () => {
      ...
      it('adds multiple ext functions with simple dependencies', (done) => {
        ...
        server.select('0').route({
          method: 'GET',
          path: '/',
          handler: handler
        });
        server.select('1').route({
          method: 'GET',
          path: '/',
          handler: handler
        });
        server.select('2').route({
          method: 'GET',
          path: '/',
          handler: handler
        });
      ...
      it('adds multiple ext functions with complex dependencies', (done) => {
        const pluginCurrier = function(num, deps) {
          ...
          plugin.attributes = {name: 'deps' + num};
          return plugin;
        };
        const handler = function(request, reply) {
        ...
        server.register([pluginCurrier(1, {after: 'deps2'}), pluginCurrier(2), pluginCurrier(3, {before: ['deps1', 'deps2']})], (err) => {
          expect(err).to.not.exist();
          server.initialize((err) => {
            expect(err).to.not.exist();
            server.inject('/', (res) => {
      ...
      it('combine route extensions', (done) => {
        ...
        const plugin = function(srv, options, next) {
          srv.route({
            ...
            path: '/',
            config: {
              ext: {onPreAuth: {method: function(request, reply) {
                    request.app.x += '2';
                    return reply.continue();
                  }}},
              handler: function(request, reply) {
  ...
  internals.plugins = {
    auth: function(server, options, next) {
      const scheme = function(srv, authOptions) {
        ...
        return {authenticate: function(request, reply) {
    ...
    test1: function(server, options, next) {
      ...
      server.select('test').route({
        path: '/test1',
        method: 'GET',
        handler: handler
      });
      server.expose({add: function(a, b) {
          return a + b;
        }});
  ...
  internals.plugins.auth.attributes = {name: 'auth'};
  internals.plugins.child.attributes = {name: 'child'};
  internals.plugins.deps1.attributes = {name: 'deps1'};
  internals.plugins.deps2.attributes = {name: 'deps2'};
  internals.plugins.deps3.attributes = {name: 'deps3'};
  internals.plugins.test1.attributes = {
    name: 'test1',
    version: '1.0.0'
  };
  internals.plugins.test2.attributes = {pkg: {
      name: 'test2',
      version: '1.0.0'
    }};
