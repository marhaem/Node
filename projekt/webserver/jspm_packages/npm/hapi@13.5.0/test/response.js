  ...
  describe('Response', () => {
    it('returns a reply', (done) => {
      const handler = function(request, reply) {
        return reply('text').type('text/plain').charset('ISO-8859-1').ttl(1000).header('set-cookie', 'abc=123').state('sid', 'abcdefg123456').state('other', 'something', {isSecure: true}).unstate('x').header('Content-Type', 'text/plain; something=something').header('vary', 'x-control').header('combo', 'o').header('combo', 'k', {
          append: true,
          separator: '-'
        }).header('combo', 'bad', {override: false}).code(200);
      };
    ...
    describe('charset()', () => {
      ...
      it('sets type inside marshal', (done) => {
        const handler = function(request, reply) {
          ...
          return reply(request.generateResponse({value: 'text'}, {
            variety: 'test',
            marshal
          }));
        };
    ...
    describe('_marshal()', () => {
      it('emits request-error when view file for handler not found', (done) => {
        ...
        server.route({
          method: 'GET',
          path: '/{param}',
          handler: {view: 'templates/invalid'}
        });
      ...
      it('returns a response with options', (done) => {
        const handler = function(request, reply) {
          return reply({
            a: 1,
            b: 2
          }).type('application/x-test').spaces(2).replacer(['a']).suffix('\n');
        };
        const server = new Hapi.Server();
        server.connection();
        server.route({
      ...
      it('returns a response with options (different order)', (done) => {
        const handler = function(request, reply) {
          return reply({
            a: 1,
            b: 2
          }).type('application/x-test').replacer(['a']).suffix('\n').spaces(2);
        };
        const server = new Hapi.Server();
        server.connection();
        server.route({
