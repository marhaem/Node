...
describe('Methods', () => {
  ...
  it('caches method value', (done) => {
    ...
    const server = new Hapi.Server();
    server.connection();
    server.method('test', method, {cache: {
        expiresIn: 1000,
        generateTimeout: 10
      }});
    server.initialize((err) => {
  ...
  it('generates new value after cache drop', (done) => {
    ...
    const server = new Hapi.Server();
    server.connection();
    server.method('dropTest', method, {cache: {
        expiresIn: 1000,
        generateTimeout: 10
      }});
    server.initialize((err) => {
  ...
  it('errors on invalid drop key', (done) => {
    ...
    const server = new Hapi.Server();
    server.connection();
    server.method('dropErrTest', method, {cache: {
        expiresIn: 1000,
        generateTimeout: 10
      }});
    server.initialize((err) => {
  ...
  it('returns a valid result when calling a method when using the cache', (done) => {
    ...
    server.initialize((err) => {
      ...
      server.method('user', method, {cache: {
          expiresIn: 1000,
          generateTimeout: 10
        }});
  ...
  it('supports empty key method', (done) => {
    ...
    server.method('tos', method, {cache: {
        expiresIn: 2000,
        generateTimeout: 10
      }});
    server.initialize((err) => {
