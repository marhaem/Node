...
describe('validation', () => {
  it('validates valid input', (done) => {
    ...
    server.route({
      ...
      config: {validate: {query: {a: Joi.number()}}}
    });
  ...
  it('validates both params and query', (done) => {
    ...
    server.route({
      ...
      config: {validate: {
          query: {a: Joi.number().integer().min(0).default(0)},
          params: {x: Joi.number()}
        }}
    });
  ...
  it('validates valid input using context', (done) => {
    ...
    server.route({
      ...
      config: {validate: {query: {verbose: Joi.boolean().when('$params.user', {
              is: Joi.exist(),
  ...
  it('validates valid input using auth context', (done) => {
    ...
    server.route({
      ...
      config: {validate: {query: {me: Joi.boolean().when('$auth.credentials.name', {
              is: Joi.ref('$params.user'),
              otherwise: Joi.forbidden()
            })}}}
    });
  ...
  it('retains custom validation error', (done) => {
    ...
    server.route({
      ...
      config: {validate: {query: {a: Joi.number().error(Boom.forbidden())}}}
    });
    server.inject('/?a=abc', (res) => {
  ...
  it('casts input to desired type', (done) => {
    ...
    server.route({
      ...
      config: {validate: {params: {seq: Joi.number()}}}
    });
  ...
  it('validates valid input (Object root)', (done) => {
    ...
    server.route({
      ...
      config: {validate: {query: Joi.object({a: Joi.string().min(2)})}}
    });
    server.inject('/?a=123', (res) => {
      expect(res.statusCode).to.equal(200);
      done();
  ...
  it('validates non-object payload', (done) => {
    ...
    server.route({
      ...
      config: {validate: {payload: Joi.number()}}
    });
    server.inject({
      method: 'POST',
      url: '/',
  ...
  it('fails on invalid input', (done) => {
    ...
    server.route({
      ...
      config: {validate: {query: {a: Joi.string().min(2)}}}
    });
  ...
  it('fails on invalid payload', (done) => {
    ...
    server.route({
      ...
      config: {validate: {payload: {a: Joi.string().min(8)}}}
    });
    server.inject({
      method: 'POST',
      url: '/',
  ...
  it('fails on text input', (done) => {
    ...
    server.route({
      ...
      config: {validate: {payload: {a: Joi.string().min(2)}}}
    });
    server.inject({
      method: 'POST',
      url: '/?a=1',
  ...
  it('fails on null input', (done) => {
    ...
    server.route({
      ...
      config: {validate: {payload: {a: Joi.string().required()}}}
    });
    server.inject({
      method: 'POST',
      url: '/',
  ...
  it('validates response', (done) => {
    ...
    server.route({
      method: 'GET',
      path: '/',
      config: {response: {schema: {some: Joi.string()}}},
      handler: handler
    });
    server.inject('/', (res1) => {
      expect(res1.statusCode).to.equal(200);
  ...
  it('validates response with context', (done) => {
    ...
    server.route({
      method: 'GET',
      path: '/',
      config: {response: {schema: Joi.object({
            some: Joi.string(),
            more: Joi.string()
          }).when('$query.user', {
            is: 'admin',
            otherwise: Joi.object({more: Joi.forbidden()})
          })}},
      handler: handler
    });
  ...
  it('validates and modifies response', (done) => {
    ...
    server.route({
      ...
      path: '/',
      config: {response: {
          schema: Joi.object({a: Joi.number()}).options({stripUnknown: true}),
          modify: true
        }},
      handler: handler
    });
  ...
  it('validates response using custom validation function', (done) => {
    ...
    server.route({
      method: 'GET',
      path: '/',
      config: {response: {schema: function(value, options, next) {
  ...
  it('validates string response', (done) => {
    ...
    server.route({
      method: 'GET',
      path: '/',
      config: {response: {schema: Joi.string().min(5)}},
      handler: handler
    });
    server.inject('/', (res1) => {
