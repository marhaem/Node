...
internals.marshal = function(request, next) {
  ...
  if (response.statusCode !== 304 && (request.method === 'get' || request.method === 'head')) {
    if (response.headers.etag && request.headers['if-none-match']) {
      ...
      for (let i = 0; i < ifNoneMatch.length; ++i) {
        ...
        if (etag === response.headers.etag) {
          ...
          if (etag === etagBase + '-gzip"' || etag === etagBase + '-deflate"') {
            response.code(304);
            break;
          }
        }
      ...
      if (ifModifiedSinceHeader && lastModifiedHeader) {
...
internals.transmit = function(response, callback) {
  ...
  let compressor = null;
  if (encoding && length !== 0 && response._isPayloadSupported()) {
    delete response.headers['content-length'];
  ...
  if (isInjection) {
    request.raw.res._hapi = {request: request};
...
internals.content = function(response, postMarshal) {
  ...
  } else if ((!response._contentType || !postMarshal) && response.settings.charset && type.match(/^(?:text\/)|(?:application\/(?:json)|(?:javascript))/)) {
    const hasParams = (type.indexOf(';') !== -1);
    if (!hasParams || !type.match(/[; ]charset=/)) {
