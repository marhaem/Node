...
(function(process) {
  ...
  const internals = {};
  internals.schema = Joi.alternatives([Joi.string(), Joi.func(), Joi.object({
  ...
  internals.marshal = function(response, next) {
    if (!response.source.settings.lookupCompressed || !response.request.connection.settings.compression || response.request.info.acceptEncoding !== 'gzip') {
  ...
  internals.addContentRange = function(response, callback) {
    ...
    let range = null;
    if (Hoek.reach(request.route.settings, 'response.ranges') !== false) {
