...
exports.handler = function(route, options) {
  ...
  const normalized = (Array.isArray(settings.path) ? normalize(settings.path) : []);
  ...
  const handler = (request, reply) => {
    ...
    Items.serial(paths, (baseDir, nextPath) => {
      ...
      File.load(path, request, fileOptions, (response) => {
        ...
        if (indexNames.length === 0 && !settings.listing) {
          return reply(Boom.forbidden());
        }
        if (settings.redirectToSlash !== false && !request.connection.settings.router.stripTrailingSlash && !hasTrailingSlash) {
...
internals.generateListing = function(path, resource, selection, hasTrailingSlash, settings, request, reply) {
  Fs.readdir(path, (err, files) => {
    ...
    for (let i = 0; i < files.length; ++i) {
      if (settings.showHidden || !internals.isFileHidden(files[i])) {
