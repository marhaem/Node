...
internals.checkItems = function(items, wasArray, state, options) {
  ...
  for (var v = 0,
    ...
    for (var i = 0,
      ...
      res = this._inner.exclusions[i]._validate(item, localState, {});
      if (!res.errors) {
...
internals.Array.prototype.describe = function() {
  ...
  if (this._inner.ordereds.length) {
    ...
    for (var o = 0,
        ol = this._inner.ordereds.length; o < ol; ++o) {
  ...
  if (this._inner.items.length) {
    description.items = [];
    for (var i = 0,
        il = this._inner.items.length; i < il; ++i) {
