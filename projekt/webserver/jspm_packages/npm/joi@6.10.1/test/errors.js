...
describe('errors', function() {
  ...
  it('returns a full path to an error value on an object', function(done) {
    var schema = {x: Joi.array().items({x: Joi.number()})};
    var input = {x: [{x: 1}, {x: 'a'}]};
  ...
  it('allows html escaping', function(done) {
    Joi.string().options({language: {
        root: 'blah',
        label: 'bleh'
      }}).validate(4, function(err, value) {
  ...
  describe('#annotate', function() {
    ...
    it('annotates error within array', function(done) {
      var object = {a: [1, 2, 3, 4, 2, 5]};
      var schema = {a: Joi.array().items(Joi.valid(1, 2))};
      Joi.validate(object, schema, {abortEarly: false}, function(err, value) {
        expect(err).to.exist();
    ...
    it('annotates error within array multiple times on the same element', function(done) {
      var object = {a: [2, 3, 4]};
      var schema = {a: Joi.array().items(Joi.number().min(4).max(2))};
      Joi.validate(object, schema, {abortEarly: false}, function(err, value) {
        expect(err).to.exist();
    ...
    it('annotates error within array when it is an object', function(done) {
      var object = {a: [{b: 2}]};
      var schema = {a: Joi.array().items(Joi.number())};
      Joi.validate(object, schema, {abortEarly: false}, function(err, value) {
        expect(err).to.exist();
    ...
    it('displays alternatives fail as a single line', function(done) {
      var schema = {x: [Joi.string(), Joi.number(), Joi.date()]};
    ...
    it('annotates circular input', function(done) {
      var schema = {x: Joi.object({y: Joi.object({z: Joi.number()})})};
      var input = {};
      input.x = input;
      Joi.validate(input, schema, function(err, value) {
        expect(err).to.exist();
