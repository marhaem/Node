  ...
  describe('any', function() {
    describe('#equal', function() {
      it('validates valid values', function(done) {
        Helper.validate(Joi.equal(4), [[4, true], [5, false]], done);
      });
    });
    describe('#not', function() {
      it('validates invalid values', function(done) {
        Helper.validate(Joi.not(5), [[4, true], [5, false]], done);
      });
    });
    describe('#exist', function() {
      it('validates required values', function(done) {
        Helper.validate(Joi.exist(), [[4, true], [undefined, false]], done);
      });
    });
    describe('#strict', function() {
      it('validates without converting', function(done) {
        var schema = Joi.object({array: Joi.array().items(Joi.string().min(5), Joi.number().min(3))}).strict();
        Helper.validate(schema, [[{array: ['12345']}, true], [{array: ['1']}, false], [{array: [3]}, true], [{array: ['12345', 3]}, true], [{array: ['3']}, false], [{array: [1]}, false]], done);
      });
      it('can be disabled', function(done) {
        var schema = Joi.object({array: Joi.array().items(Joi.string().min(5), Joi.number().min(3))}).strict().strict(false);
        Helper.validate(schema, [[{array: ['12345']}, true], [{array: ['1']}, false], [{array: [3]}, true], [{array: ['12345', 3]}, true], [{array: ['3']}, true], [{array: [1]}, false]], done);
      });
    });
    ...
    describe('#raw', function() {
      it('gives the raw input', function(done) {
        var tests = [[Joi.array(), '[1,2,3]'], [Joi.binary(), 'abc'], [Joi.boolean(), 'false'], [Joi.date().format('YYYYMMDD'), '19700101'], [Joi.number(), '12'], [Joi.object(), '{ "a": 1 }'], [Joi.any().strict(), 'abc']];
        tests.forEach(function(test) {
          var baseSchema = test[0];
          var input = test[1];
          var schemas = [baseSchema.raw(), baseSchema.raw(true), baseSchema.options({raw: true})];
    ...
    describe('#default', function() {
      ...
      it('throws when value is a method and no description is provided', function(done) {
        expect(function() {
          Joi.object({foo: Joi.string().default(function() {
              return 'test';
            })});
        }).to.throw();
        done();
      ...
      it('sets the value when passing a method', function(done) {
        var schema = Joi.object({foo: Joi.string().default(function() {
            return 'test';
          }, 'testing')});
        var input = {};
        schema.validate(input, function(err, value) {
      ...
      it('executes the default method each time validate is called', function(done) {
        var count = 0;
        var schema = Joi.object({foo: Joi.number().default(function() {
            return ++count;
          }, 'incrementer')});
        var input = {};
        schema.validate(input, function(err, value) {
      ...
      it('sets value based on condition (outer)', function(done) {
        var schema = Joi.object({
          a: Joi.boolean(),
          b: Joi.boolean().default(false).when('a', {
            is: true,
            then: Joi.required(),
      ...
      it('creates deep defaults', function(done) {
        ...
        Helper.validate(schema, [[undefined, true, null, {
          a: 42,
          b: {c: true}
        }], [{a: 24}, true, null, {
          a: 24,
      ...
      it('should not affect objects other than object when called without an argument', function(done) {
        var schema = Joi.object({a: Joi.number().default()}).default();
        Helper.validate(schema, [[undefined, true, null, {}], [{a: 24}, true, null, {a: 24}]], done);
      });
    ...
    describe('#optional', function() {
      it('validates optional with default required', function(done) {
        ...
        Helper.validate(schema, [[{a: 5}, false], [{
          a: 5,
          b: 6
        }, false], [{
          a: 5,
    ...
    describe('#forbidden', function() {
      it('validates forbidden', function(done) {
        ...
        Helper.validate(schema, [[{a: 5}, true], [{
          a: 5,
          b: 6
        }, false], [{a: 'a'}, false], [{}, true], [{b: undefined}, true], [{b: null}, false]], done);
      });
    });
    ...
    describe('#meta', function() {
      it('sets the meta', function(done) {
        ...
        b = b.meta({other: true});
        expect(b.describe().meta).to.deep.equal([meta, {other: true}]);
        done();
      });
    ...
    describe('#concat', function() {
      ...
      it('merges two schemas (settings)', function(done) {
        ...
        Helper.validate(a, [[1, true], ['1', true]]);
        Helper.validate(a.concat(b), [[1, true], ['1', false]], done);
      });
      it('merges two schemas (valid)', function(done) {
        var a = Joi.string().valid('a');
        var b = Joi.string().valid('b');
        Helper.validate(a, [['a', true], ['b', false]]);
        Helper.validate(b, [['b', true], ['a', false]]);
        Helper.validate(a.concat(b), [['a', true], ['b', true]], done);
      });
      it('merges two schemas (invalid)', function(done) {
        var a = Joi.string().invalid('a');
        var b = Joi.invalid('b');
        Helper.validate(a, [['b', true], ['a', false]]);
        Helper.validate(b, [['a', true], ['b', false]]);
        Helper.validate(a.concat(b), [['a', false], ['b', false]], done);
      });
      it('merges two schemas (valid/invalid)', function(done) {
        ...
        Helper.validate(a, [['a', true], ['b', false]]);
        Helper.validate(b, [['b', true], ['a', false]]);
        Helper.validate(a.concat(b), [['a', false], ['b', true]], done);
      });
      it('merges two schemas (tests)', function(done) {
        var a = Joi.number().min(5);
        var b = Joi.number().max(10);
        Helper.validate(a, [[4, false], [11, true]]);
        Helper.validate(b, [[6, true], [11, false]]);
        Helper.validate(a.concat(b), [[4, false], [6, true], [11, false]], done);
      });
      it('merges two schemas (flags)', function(done) {
        var a = Joi.string().valid('a');
        var b = Joi.string().insensitive();
        Helper.validate(a, [['a', true], ['A', false], ['b', false]]);
        Helper.validate(a.concat(b), [['a', true], ['A', true], ['b', false]], done);
      });
      ...
      it('merges two objects (any key + specific key)', function(done) {
        ...
        Helper.validate(a, [[{b: 1}, true], [{b: 2}, true]]);
        Helper.validate(b, [[{b: 1}, true], [{b: 2}, false]]);
        Helper.validate(a.concat(b), [[{b: 1}, true], [{b: 2}, false]]);
        Helper.validate(b.concat(a), [[{b: 1}, true], [{b: 2}, false]], done);
      });
      it('merges two objects (no key + any key)', function(done) {
        var a = Joi.object({});
        var b = Joi.object();
        Helper.validate(a, [[{}, true], [{b: 2}, false]]);
        Helper.validate(b, [[{}, true], [{b: 2}, true]]);
        Helper.validate(a.concat(b), [[{}, true], [{b: 2}, false]]);
        Helper.validate(b.concat(a), [[{}, true], [{b: 2}, false]], done);
      });
      it('merges two objects (key + key)', function(done) {
        var a = Joi.object({a: 1});
        var b = Joi.object({b: 2});
        Helper.validate(a, [[{a: 1}, true], [{b: 2}, false]]);
        Helper.validate(b, [[{a: 1}, false], [{b: 2}, true]]);
        Helper.validate(a.concat(b), [[{a: 1}, true], [{b: 2}, true]]);
        Helper.validate(b.concat(a), [[{a: 1}, true], [{b: 2}, true]], done);
      });
    ...
    describe('#when', function() {
      ...
      it('makes peer required', function(done) {
        ...
        Helper.validate(schema, [[{b: 5}, false], [{b: 6}, true], [{a: 'b'}, true], [{
          b: 5,
          a: 'x'
        }, true]], done);
      });
    ...
    describe('#requiredKeys', function() {
      it('should set keys as required', function(done) {
        ...
        Helper.validate(schema, [[{}, false], [{a: 0}, false], [{
          a: 0,
          b: 0
        }, false], [{
          a: 0,
      ...
      it('should throw on unknown key', function(done) {
        expect(function() {
          Joi.object({
            a: 0,
            b: 0
          }).requiredKeys('a', 'c', 'b', 'd', 'd.e.f');
        }).to.throw(Error, 'unknown key(s) c, d');
        expect(function() {
          Joi.object({
            a: 0,
            b: 0
          }).requiredKeys('a', 'b', 'a.c.d');
    ...
    describe('#optionalKeys', function() {
      it('should set keys as optional', function(done) {
        ...
        Helper.validate(schema, [[{}, true], [{a: 0}, true], [{
          a: 0,
          b: 0
        }, true]], done);
      });
    ...
    describe('#empty', function() {
      it('should void values when considered empty', function(done) {
        ...
        Helper.validate(schema, [[undefined, true, null, undefined], ['abc', true, null, 'abc'], ['', true, null, undefined]], done);
      });
      it('should override any previous empty', function(done) {
        var schema = Joi.string().empty('').empty('abc');
        Helper.validate(schema, [[undefined, true, null, undefined], ['abc', true, null, undefined], ['', false, null, '"value" is not allowed to be empty'], ['def', true, null, 'def']], done);
      });
      it('should be possible to reset the empty value', function(done) {
        var schema = Joi.string().empty('').empty();
        Helper.validate(schema, [[undefined, true, null, undefined], ['abc', true, null, 'abc'], ['', false, null, '"value" is not allowed to be empty']], done);
      });
      ...
      it('should work with dependencies', function(done) {
        ...
        Helper.validate(schema, [[{}, false, null, '"value" must contain at least one of [a, b]'], [{a: ''}, false, null, '"value" must contain at least one of [a, b]'], [{a: 'a'}, true, null, {a: 'a'}], [{
          a: '',
          b: 'b'
        }, true, null, {b: 'b'}]], done);
      });
