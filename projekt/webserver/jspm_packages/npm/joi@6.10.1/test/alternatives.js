...
describe('alternatives', function() {
  ...
  it('applies modifiers when higher priority converts', function(done) {
    var schema = Joi.object({a: [Joi.number(), Joi.string()]});
  ...
  it('applies modifiers when lower priority valid is a match', function(done) {
    var schema = Joi.object({a: [Joi.number(), Joi.valid('5')]});
    schema.validate({a: '5'}, function(err, value) {
      expect(err).to.not.exist();
      expect(value.a).to.equal(5);
      done();
  ...
  describe('#when', function() {
    ...
    it('validates only then', function(done) {
      var schema = {
        a: Joi.alternatives().when(Joi.ref('b'), {
          is: 5,
          then: 'x'
        }).try('z'),
        b: Joi.any()
    ...
    it('validates when is is null', function(done) {
      ...
      Helper.validate(schema, [[{a: 1}, true], [{a: 'y'}, false], [{
        a: 'x',
        b: null
      }, true], [{
        a: 'y',
    ...
    it('validates when empty value', function(done) {
      ...
      Helper.validate(schema, [[{b: false}, true], [{b: true}, true]], done);
    });
    it('validates when missing value', function(done) {
      ...
      Helper.validate(schema, [[{a: 1}, true], [{}, false], [{b: 1}, false], [{
        a: 1,
        b: 1
      }, true], [{
        a: 1,
  ...
  describe('#describe', function() {
    ...
    it('describes inherited fields (from any)', function(done) {
      var schema = Joi.alternatives().try('a').description('d').example('a').meta('b').meta('c').notes('f').tags('g');
      var outcome = {
        type: 'alternatives',
        description: 'd',
        notes: ['f'],
