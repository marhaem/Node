...
(function(process) {
  ...
  describe('#getFileName()', function() {
    it('should return the default pgpass file', function() {
      ...
      assert.equal(helper.getFileName(env), process.platform === 'win32' ? 'C:\\tmp\\postgresql\\pgpass.conf' : '/tmp/.pgpass');
    });
  ...
  describe('#usePgPass()', function() {
    ...
    Object.keys(testResults).forEach(function(octPerm) {
      ...
      var msg = util.format('should consider permission %s %s', octPerm, res ? 'secure' : 'not secure');
      it(msg, function() {
  ...
  describe('#parseLine()', function() {
    ...
    it('should ignore too short lines', function() {
      var tests = ['::::', 'host:port', 'host:port:database', 'host:port:database:'];
  ...
  describe('#isValidEntry()', function() {
    ...
    it('should find invalid entries', function() {
      assert(!helper.isValidEntry({'host': ''}));
      assert(!helper.isValidEntry({
        'host': 'host',
        'port': '100',
        'database': 'database',
  ...
  describe('#getPassword()', function() {
    var creds = 'host1:100:database1:user1:thepassword1' + '\n' + '*:*:database2:*:thepassword2' + '\n';
    ;
    var conn1 = {
      'host': 'host1',
      'port': 100,
