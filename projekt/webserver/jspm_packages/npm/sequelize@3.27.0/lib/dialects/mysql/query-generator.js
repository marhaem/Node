...
var QueryGenerator = {
  ...
  getForeignKeyQuery: function(table, columnName) {
    ...
    return ['SELECT CONSTRAINT_NAME as constraint_name', 'FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE', 'WHERE (REFERENCED_TABLE_NAME = ' + wrapSingleQuote(tableName), 'AND REFERENCED_COLUMN_NAME = ' + wrapSingleQuote(columnName), ') OR (TABLE_NAME = ' + wrapSingleQuote(tableName), 'AND COLUMN_NAME = ' + wrapSingleQuote(columnName), ')'].join(' ');
  },
