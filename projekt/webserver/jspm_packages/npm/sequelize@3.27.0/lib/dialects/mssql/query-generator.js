...
var QueryGenerator = {
  ...
  dropTableQuery: function(tableName) {
    ...
    var values = {table: this.quoteTable(tableName)};
    return Utils._.template(query)(values).trim() + ';';
  },
  ...
  upsertQuery: function(tableName, insertValues, updateValues, where, rawAttributes, options) {
    ...
    var sourceTableQuery = 'VALUES(' + insertValuesEscaped + ')';
    ...
    if (needIdentityInsertWrapper) {
      query = ['SET IDENTITY_INSERT', this.quoteTable(tableName), 'ON;', query, 'SET IDENTITY_INSERT', this.quoteTable(tableName), 'OFF;'].join(' ');
    }
    return query;
  },
  deleteQuery: function(tableName, where, options) {
    ...
    var limit = '',
        query = 'DELETE<%= limit %> FROM <%= table %><%= where %>; ' + 'SELECT @@ROWCOUNT AS AFFECTEDROWS;';
