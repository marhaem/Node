  ...
  if (self.sequelize.options.dialect === 'postgres') {
    ...
    for (i = 0; i < keyLen; i++) {
      if (attributes[keys[i]].type instanceof DataTypes.ENUM) {
        ...
        promises.push(self.sequelize.query(sql, _.assign({}, options, {
          plain: true,
          raw: true,
    ...
    return Promise.all(promises).then(function(results) {
      ...
      if (!tableName.schema && (options.schema || (!!model && model.$schema))) {
        tableName = self.QueryGenerator.addSchema({
          tableName: tableName,
          $schema: (!!model && model.$schema) || options.schema
        });
      }
      attributes = self.QueryGenerator.attributesToSQL(attributes, {context: 'createTable'});
...
QueryInterface.prototype.dropAllEnums = function(options) {
  ...
  return this.pgListEnums(null, options).map(function(result) {
    return self.sequelize.query(self.QueryGenerator.pgEnumDrop(null, null, self.QueryGenerator.pgEscapeAndQuote(result.enum_name)), _.assign({}, options, {raw: true}));
  });
};
...
QueryInterface.prototype.renameColumn = function(tableName, attrNameBefore, attrNameAfter, options) {
  ...
  return this.describeTable(tableName, options).then(function(data) {
    ...
    if (this.sequelize.options.dialect === 'sqlite') {
      ...
      var sql = this.QueryGenerator.renameColumnQuery(tableName, attrNameBefore, this.QueryGenerator.attributesToSQL(_options));
      return this.sequelize.query(sql, options);
    }
  }.bind(this));
};
...
QueryInterface.prototype.setAutocommit = function(transaction, value, options) {
  ...
  var sql = this.QueryGenerator.setAutocommitQuery(value, {parent: transaction.parent});
  if (!sql)
    return Promise.resolve();
  return this.sequelize.query(sql, options);
};
QueryInterface.prototype.setIsolationLevel = function(transaction, value, options) {
  ...
  options = _.assign({}, options, {transaction: transaction.parent || transaction});
  var sql = this.QueryGenerator.setIsolationLevelQuery(value, {parent: transaction.parent});
  if (!sql)
    return Promise.resolve();
  return this.sequelize.query(sql, options);
};
