...
var shimmer = require('../index');
var outsider = 0;
function counter() {
  return ++outsider;
}
var generator = {inc: counter};
...
test('wrap called with no arguments', function(t) {
  t.plan(2);
  var mock = sinon.expectation.create('logger').withExactArgs('no original function undefined to wrap').once();
  shimmer({logger: mock});
  t.doesNotThrow(function() {
    shimmer.wrap();
...
test('wrap called with non-function original', function(t) {
  t.plan(2);
  var mock = sinon.expectation.create('logger').withExactArgs('original object and wrapper must be functions').once();
  shimmer({logger: mock});
  t.doesNotThrow(function() {
...
test('wrap called with non-function wrapper', function(t) {
  t.plan(2);
  var mock = sinon.expectation.create('logger').withArgs('original object and wrapper must be functions').once();
  shimmer({logger: mock});
  t.doesNotThrow(function() {
